...
export default function Instructors() {
  ...
  const [isAlineExpanded, setIsAlineExpanded] = useState(false);
  const [isPedroExpanded, setIsPedroExpanded] = useState(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-[-80px] md:mt-[-120px] px-4 md:px-12 relative z-10">
            ...
            >
              ...
              <div className="flex-grow flex flex-col">
                <p className={`text-zinc-600 leading-relaxed text-justify text-sm ${!isAlineExpanded ? 'max-md:line-clamp-[7]' : ''}`}>
                  Com uma trajetória marcada por dedicação, disciplina e profundo conhecimento da arte suave, Aline Coelho é uma das grandes referências técnicas da equipe. Faixa preta há 24 anos, construiu uma carreira sólida dentro do Jiu Jitsu, acumulando experiência tanto na formação de atletas quanto no desenvolvimento técnico de praticantes de diferentes níveis.
                  <br /><br />
                ...
                {!isAlineExpanded && (
                  <button 
                    onClick={() => setIsAlineExpanded(true)}
                    className="md:hidden text-red-600 font-bold text-xs mt-2 text-left hover:text-red-700 transition-colors"
                  >
                    ... continuar lendo
                  </button>
                )}
            ...
            >
              ...
              <div className="flex-grow flex flex-col">
                <div className={`text-zinc-400 leading-relaxed text-justify text-sm space-y-4 ${!isPedroExpanded ? 'max-md:line-clamp-[7]' : ''}`}>
                  <p>
                ...
                {!isPedroExpanded && (
                  <button 
                    onClick={() => setIsPedroExpanded(true)}
                    className="md:hidden text-red-500 font-bold text-xs mt-2 text-left hover:text-red-400 transition-colors"
                  >
                    ... continuar lendo
                  </button>
                )}
                ...
                <button 
                  ...
                  className="inline-flex items-center gap-2 text-red-500 font-bold hover:text-red-400 transition-colors group mt-4"
                >
